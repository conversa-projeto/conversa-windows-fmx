unit Conversa.Notificacao.Item.Mensagem;
...
type
  TNotificacaoItemMensagem = class(TNotificacaoItemBase)
    rctFundo: TRectangle;
    txtHora: TText;
    lytTopo: TLayout;
    lytCentro: TLayout;
  ...
  public
    class function New(AOwner: TFmxObject): TNotificacaoItemMensagem;
...
class function TNotificacaoItemMensagem.New(AOwner: TFmxObject): TNotificacaoItemMensagem;
begin
  Sleep(1);
  Result := TNotificacaoItemMensagem.Create(AOwner);
  Result.Name := 'TNotificacaoItemMensagem_'+ FormatDateTime('yyyymmddHHnnsszzzz', Now);
...
procedure TNotificacaoItemMensagem.pbTextoPaint(Sender: TObject; Canvas: TCanvas);
const
...
procedure TNotificacaoItemMensagem.AtualizarConteudo(AChatId: Integer; AConteudos: TArray<TMensagemNotificacao>);
begin
...
destructor TNotificacaoItemMensagem.Destroy;
begin
  TMessageManager.DefaultManager.Unsubscribe(TEventoMudancaStatusUsuarioSO, StatusIniciarAnimacao);
  inherited;
end;

procedure TNotificacaoItemMensagem.Fechar;
begin
  TNotificacaoManager.Fechar(TTipoNotificacao.Mensagem, FChatId);
end;

procedure TNotificacaoItemMensagem.FloatAnimationFinish(Sender: TObject);
begin
  Fechar;
end;

procedure TNotificacaoItemMensagem.lytCloseButtonClick(Sender: TObject);
begin
  Fechar;
end;

procedure TNotificacaoItemMensagem.rctFundoClick(Sender: TObject);
begin
  Fechar;
...
procedure TNotificacaoItemMensagem.RegistrarEvento;
begin
  TMessageManager.DefaultManager.SubscribeToMessage(TEventoMudancaStatusUsuarioSO, StatusIniciarAnimacao);
end;

procedure TNotificacaoItemMensagem.StatusIniciarAnimacao(const Sender: TObject; const M: TMessage);
begin
...
procedure TNotificacaoItemMensagem.AtualizarAltura(iTexto: Single);
var
  iNova: Single;
begin
